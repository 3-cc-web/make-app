    ...
    return (
        ...
        <main className="flex-1 px-4 pt-4 pb-24">
            ...
            <select
                ...
                onChange={(e) => setSelectedCategory(Number(e.target.value))} // 選択が変わったらstateを更新
            ...
            {filteredProducts.map((product) => (
                ...
                >
                {/* 商品名。長い場合は省略 */}
                <span className="h-[35px] text-sm text-center leading-tight line-clamp-2 w-full">
                    {product.name}
                </span>
                {/* 画像エリア。imageがnullのときはグレーのプレースホルダー */}
                <div className="w-full aspect-square bg-white border border-gray-200 rounded-md flex items-center justify-center overflow-hidden">
                    {product.image_path ? (
                    <img src={getImageUrl(product.image_path)} alt={product.name} className="w-auto h-[90%] object-cover" />
                    ) : (
            ...
            >
                ...
                <>
                    ...
                    <span className={`text-xs px-3 py-1 rounded-full font-medium ${categoryColors[modalProduct.category] ?? "bg-gray-100 text-gray-600"}`}>
                        {categories?.find(c => c.id === modalProduct.category)?.name}
                    </span>
                    </div>

                    {/* 画像エリア */}
                    <div className="w-full aspect-square bg-white border border-gray-200 rounded-lg flex items-center justify-center mb-4">
                    {modalProduct.image_path ? (
                        <img src={getImageUrl(modalProduct.image_path)} alt={modalProduct.name} className="w-auto h-[90%] object-cover rounded-lg" />
                    ) : (
                    ...
                    {/* 成分エリア */}
                    <div className="flex justify-center items-center flex-col mb-3 text-xs text-gray-600">
                      {[
                        modalProduct.ingredient1,
                        modalProduct.ingredient2,
                        modalProduct.ingredient3,
                        modalProduct.ingredient4,
                        modalProduct.ingredient5,
                        modalProduct.ingredient6,
                        modalProduct.ingredient7,
                        modalProduct.ingredient8,
                        modalProduct.ingredient9,
                        modalProduct.ingredient10
                      ]
                      .filter(Boolean)
                      .join(" / ")}
                    </div>
